...
class Model:
    ...
    def train_batch(self, input_vals):
        ...
        reconstructor_loss = self.autoencoder.criterion_NLLLoss(reconstructor_output, input_ndx.reshape((-1,)))
        ...
        wandb.log({"reconstructor_LR": self.autoencoder.reconstructor_lr_scheduler.get_last_lr()[0]})
        ...
        wandb.log({"generator_LR": self.autoencoder.generator_lr_scheduler.get_last_lr()[0]})
        # train discriminator
        ...
        wandb.log({"discriminator_LR": self.autoencoder.discriminator_lr_scheduler.get_last_lr()[0]})
        ...
        classifier_loss = self.autoencoder.criterion_MSELoss(classifier_output, classifier_target)
        classifier_loss.backward()
        self.autoencoder.classifier_optimizer.step()
        wandb.log({"classifier_loss": classifier_loss.item()})
        wandb.log({"classifier_LR": self.autoencoder.classifier_lr_scheduler.get_last_lr()[0]})
